...
// Add new string to ArrayValue, by example "one, two, three"
...
func main() {
	// Extracting flag values methods returning pointers
	retry := flag.Int("retry", -1, "Defines max retry count")

	// Read the flag usign the XXXVar function.
	...
	for retryCount < *retry {
		logger.Println("Retrying connecting")
